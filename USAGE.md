...
Требуется Python 3.10+ и базовые инструменты сборки. На Debian/Ubuntu:
...
Проект публикует исполняемый скрипт `secaudit`. Если нужно вызвать CLI без установки, можно использовать `python3 -m secaudit.main` или `python3 main.py` из корня репозитория.
...
В секции `vars` описываются значения по умолчанию, уровневые настройки и файлы с переменными. Движок выстраивает контекст следующим образом:

1. Значения `defaults` из текущего профиля.
2. Файлы, перечисленные в `vars.files` (обязательные) и `vars.optional_files` (опциональные). В пути разрешаются шаблоны `{{ level }}`.
...
3. Добавляйте проверки с уникальными `id`; запустите `pytest`, чтобы убедиться в отсутствии коллизий.
